...
#define GRID_BACKGROUND_INTENSITY 0.0f
#define CURR_CURVE_INTENSITY 1.0f
#define DIMMED_CURVE_INTENSITY 0.45f
#define GRID_LINE_INTENSITY 0.3f
...
int GraphWidget::getCurrCurve() const {
	return m_iCurrCurve;
}

void GraphWidget::drawGrid() const {
	if (m_iCurrCurve < 0) return;
	...
	double dRangeY = m_cdvCurveDomains[m_iCurrCurve].mag();
	// TODO: make this work for multiple curves
	double top = m_cdvCurveDomains[m_iCurrCurve].maximum() - (1 - m_rectCurrViewport.top()) * dRangeY;
	double bottom = m_cdvCurveDomains[m_iCurrCurve].minimum() + m_rectCurrViewport.bottom() * dRangeY;
	dRangeY = top - bottom;

	// Get the window dimensions
	int iWindowWidth = w(), iWindowHeight = h();
	...
	if (iLongMarkCountX > 0 && iWindowWidth > 0) {
		...
		// The length is in terms of time in seconds.
		...
		glColor3f(GRID_LINE_INTENSITY, GRID_LINE_INTENSITY,
			GRID_LINE_INTENSITY);
		glLineWidth(1);
		...
		glBegin(GL_LINES);
			...
			float timeX = (ceil(leftTime() / dLongMarkLengthX) * dLongMarkLengthX);
			do {
				// Calculate line X-coordinate, and convert to screen pixels
				int lineX = (int)((timeX - leftTime()) * w() / dRangeX);

				// Draw the line
				glVertex3i(lineX, h()-1, 0);
				glVertex3i(lineX, 0, 0);

				/*char num[10];
				glColor3f(1.0, 1.0, 1.0);
				sprintf(num, "%4f", timeX);
				glRasterPos3f(30, 30, 0);
				gl_draw(num, strlen(num));*/

				// Go to next time
				timeX += dLongMarkLengthX;
			} while (timeX <= endTime());

			// Determine starting horizontal tick
			float tickY = (ceil(bottom / dLongMarkLengthY) * dLongMarkLengthY);
			do {
				// Calculate line Y-coordinate, and convert to screen pixels
				int lineY = (int)((tickY - bottom) * h() / dRangeY);

				// Draw the line
				glVertex3i(w()-1, lineY, 0);
				glVertex3i(0, lineY, 0);

				// Go to next tick
				tickY += dLongMarkLengthY;
			} while (tickY <= top);
		glEnd();
	}
...
void GraphWidget::draw()
{
	...
	if (!valid()) {
		...
		// Set the color the screen is cleared to when glClear is called.
...
bool GraphWidget::loadModelScript(const char* szFileName)
{
	...
	iModelCurveCount = m_pcrvvCurves.size();
	...
	ifsFile >> ui->movieWidth;
	ifsFile >> ui->movieHeight;
	ifsFile >> fps;
	ui->setFPS(fps);
...
bool GraphWidget::saveModelScript(const char* szFileName) const
{
	...
	int iModelCurveCount = m_pcrvvCurves.size();

	ModelerUserInterface* ui = ModelerUserInterface::getInstance();
	ofsFile << m_fEndTime << std::endl;
	ofsFile << ui->movieWidth << std::endl;
	ofsFile << ui->movieHeight << std::endl;
	ofsFile << ui->getFPS() << std::endl;
