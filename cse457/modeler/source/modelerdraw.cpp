...
// Given 2 points in their order in from the list of points, find angle of the normal
// of their line segment and return this angle
GLfloat calcNormal(Point2d p1, Point2d p2, float PI) {
	double diffX = p2.x - p1.x;
	double diffY = p2.y - p1.y;
	// If the points are left of the y axis (as in the space needle)
	// the PI / 2 needs the opposite sign so that the normal doesn't point
	// inside of the object
	if (p1.x > 0 || p2.x > 0)
		return atan2(diffY, diffX) + PI / 2;
	else
		return atan2(diffY, diffX) - PI / 2;
}
...
void drawRevolution(double scale, float normalSize)
{
	...
	} else    
    {
		...
		for (int i = 1; i < num_pts; i++) {
			float diffX = revolution_pts[i].x - revolution_pts[i-1].x;
			float diffY = revolution_pts[i].y - revolution_pts[i-1].y;
			float len = sqrt(diffX*diffX + diffY*diffY);
			curve_length += len;
		}

		// Calculate v coordinates for each point for texture mapping
		std::vector<GLfloat> vCoords;
		...
		for (int i = 1; i < num_pts; i++) {
			...
			float len = sqrt(diffX*diffX + diffY*diffY);
			vSum += len;
			vCoords.push_back(vSum / curve_length);
		}

		// Calculate normals in xy plane, represented by angle of vector
		// Ends are calculated using normal of adjacent line seg
		// Middles are calculated by averaging normals of adjacent line segs
		std::vector<GLfloat> xyNormals;
		xyNormals.push_back(calcNormal(revolution_pts[0], revolution_pts[1], PI));
		for (int i = 1; i < num_pts - 1; i++) {
			GLfloat angle1 = calcNormal(revolution_pts[i - 1], revolution_pts[i], PI);
			GLfloat angle2 = calcNormal(revolution_pts[i], revolution_pts[i + 1], PI);
			GLfloat avg = atan2((sin(angle1) + sin(angle2)) / 2,
								(cos(angle1) + cos(angle2)) / 2);
			xyNormals.push_back(avg);
		}
		xyNormals.push_back(calcNormal(revolution_pts[num_pts-2], revolution_pts[num_pts-1], PI));
		...
		for ( int i=0; i<divisions; ++ i ) {
			...
			float theta0 = 2 * PI / divisions * i;
			float theta1 = 2 * PI / divisions * (i + 1);
			for ( int j=1; j<num_pts; ++ j ) {				
				GLfloat p1_x = (float)(revolution_pts[j - 1].x * scale * cos(theta0));
				GLfloat p1_y = (float)(revolution_pts[j - 1].y * scale + origin_y);
				GLfloat p1_z = (float)(revolution_pts[j - 1].x * scale * sin(theta0));

				GLfloat p2_x = (float)(revolution_pts[j].x * scale * cos(theta0));
				GLfloat p2_y = (float)(revolution_pts[j].y * scale + origin_y);
				GLfloat p2_z = (float)(revolution_pts[j].x * scale * sin(theta0));

				GLfloat p3_x = (float)(revolution_pts[j - 1].x * scale * cos(theta1));
				GLfloat p3_y = (float)(revolution_pts[j - 1].y * scale + origin_y);
				GLfloat p3_z = (float)(revolution_pts[j - 1].x * scale * sin(theta1));

				GLfloat p4_x = (float)(revolution_pts[j].x * scale * cos(theta1));
				GLfloat p4_y = (float)(revolution_pts[j].y * scale + origin_y);
				GLfloat p4_z = (float)(revolution_pts[j].x * scale * sin(theta1));

				// You must compute the normal directions add texture coordinates, see lecture notes
				// Using the angles calculated previously and the angle of the current rotation segment,
				// we can use trig to find a length 1 normal vector for each vertex
				float n1_x = cos(xyNormals[j - 1]) * cos(theta0);
				float n1_y = sin(xyNormals[j - 1]);
				float n1_z = cos(xyNormals[j - 1]) * sin(theta0);

				float n2_x = cos(xyNormals[j]) * cos(theta0);
				float n2_y = sin(xyNormals[j]);
				float n2_z = cos(xyNormals[j]) * sin(theta0);

				float n3_x = cos(xyNormals[j - 1]) * cos(theta1);
				float n3_y = sin(xyNormals[j - 1]);
				float n3_z = cos(xyNormals[j - 1]) * sin(theta1);

				float n4_x = cos(xyNormals[j]) * cos(theta1);
				float n4_y = sin(xyNormals[j]);
				float n4_z = cos(xyNormals[j]) * sin(theta1);

				// u coordinates for texture mapping
				float texture_u0 = 1 - theta0 / (2 * PI);
				float texture_u1 = 1 - theta1 / (2 * PI);

				// Mostly copied from help slides
				GLfloat vertices[12] = { p1_x, p1_y, p1_z, p2_x, p2_y, p2_z, p3_x, p3_y, p3_z, p4_x, p4_y, p4_z };
				GLfloat normals[12] = { n1_x, n1_y, n1_z, n2_x, n2_y, n2_z, n3_x, n3_y, n3_z, n4_x, n4_y, n4_z };
				GLfloat texture_uv[8] = { texture_u0, vCoords[j - 1], texture_u0, vCoords[j], texture_u1, vCoords[j - 1], texture_u1, vCoords[j] };
				const int indices_length = 6;
				GLuint indices[indices_length] = { 0, 1, 3, 0, 3, 2 };
				...
				if (normalSize) {
					...
					glBegin(GL_LINES);
					glVertex3f(p1_x, p1_y, p1_z);
					glVertex3f(p1_x + n1_x * normalSize, p1_y + n1_y * normalSize, p1_z + n1_z * normalSize);
					glVertex3f(p2_x, p2_y, p2_z);
					glVertex3f(p2_x + n2_x * normalSize, p2_y + n2_y * normalSize, p2_z + n2_z * normalSize);
					glEnd();
				}
			}			
		}
