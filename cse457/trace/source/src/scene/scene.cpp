...
bool Geometry::intersect(const ray&r, isect&i) const
{
    ...
    if (intersectLocal(localRay, i)) {
		...
		if (i.material != nullptr && i.material->hasNormalMap())
		{
			Vec3d c1 = i.N ^ Vec3d(0, 0, 1);
			Vec3d c2 = i.N ^ Vec3d(0, 1, 0);
			Vec3d tan = c1.length2() > c2.length2() ? c1 : c2;
			tan.normalize();
			Vec3d bit = i.N ^ tan;
			bit.normalize();

			Mat3d tbn(tan[0], tan[1], tan[2],
				bit[0], bit[1], bit[2],
				i.N[0], i.N[1], i.N[2]);
			Mat3d nbt(tan[0], bit[0], i.N[0],
				tan[1], bit[1], i.N[1],
				tan[2], bit[2], i.N[2]);
			auto mappedNormal = i.material->getNormalAt(i.uvCoordinates);
			mappedNormal.normalize();

			i.N = tbn.transpose() * mappedNormal;
			i.N.normalize();
			i.N = -i.N;
		}
