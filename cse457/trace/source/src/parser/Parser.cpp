...
void Parser::parseCylinder(Scene* scene, TransformNode* transform, const Material& mat)
{
  ...
  for( ;; )
  {
    ...
    switch( t->kind() )
    {
      ...
      case RBRACE:
         _tokenizer.Read( RBRACE );
        cylinder = new Cylinder(scene, newMat ? newMat : new Material(mat), capped);
...
void Parser::parseTrimesh(Scene* scene, TransformNode* transform, const Material& mat)
{
  ...
  for( ;; )
  {
    ...
    switch( t->kind() )
    {
	  ...
	  case TEXTURE_UV:
		_tokenizer.Read( TEXTURE_UV );
        _tokenizer.Read( EQUALS );
        _tokenizer.Read( LPAREN );
        if( RPAREN != _tokenizer.Peek()->kind() )
        {
          tmesh->addTextureUV( parseVec2d() );
          for( ;; )
          {
             const Token* nextToken = _tokenizer.Peek();
             if( RPAREN == nextToken->kind() )
...
Vec2d Parser::parseVec2d()
{
  ...
  return Vec2d( value1->value(), value2->value() );
}
  ...
  for( ;; )
  {
    ...
    switch( token->kind() )
    {
      ...
      case NAME:
         ...
         break;
	  case BUMP:
		  mat->setNormalMap(parseBumpExpression(scene));
		  break;
      case RBRACE:
        _tokenizer.Read( RBRACE );
        if( ! name.empty() )
...
TextureMap* Parser::parseBumpExpression(Scene* scene)
{
	_tokenizer.Get();
	_tokenizer.Read(EQUALS);
	string filename = _basePath;
	filename.append("/");
	filename.append(_tokenizer.Read(IDENT)->ident());
	_tokenizer.CondRead(SEMICOLON);

	return scene->getTexture(filename);
}
