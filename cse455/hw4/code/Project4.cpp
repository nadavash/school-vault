...
void MainWindow::DisplayAverageFace(QImage *displayImage, double *trainingData, int *trainingLabel, int numTrainingExamples, int patchSize)
{
	...
	int numFaces = 0;
	int numNonFaces = 0;
	std::vector<double> faceAverage(patchSize * patchSize, 0);
	std::vector<double> nonFaceAverage(patchSize * patchSize, 0);

	for (int i = 0; i < numTrainingExamples; ++i)
	{
		bool face = trainingLabel[i] == 1;
		if (face)
			++numFaces;
		else
			++numNonFaces;

		for (int j = 0; j < totalPatchSize; ++j)
		{
			if (face)
				faceAverage[j] += trainingData[totalPatchSize * i + j];
			else
				nonFaceAverage[j] += trainingData[totalPatchSize * i + j];
		}
	}
	...
	for (int i = 0; i < totalPatchSize; ++i)
	{
		faceAverage[i] /= numFaces;
		nonFaceAverage[i] /= numNonFaces;
	}
	...
	for (int r = 0; r < patchSize; ++r)
	{
		...
		{
			double facePix = faceAverage[r * patchSize + c];
			displayImage->setPixel(c, r, qRgb(facePix, facePix, facePix));

			double nonFacePix = nonFaceAverage[r * patchSize + c];
			displayImage->setPixel(c + patchSize, r, qRgb(nonFacePix, nonFacePix, nonFacePix));
		}
	}
}
