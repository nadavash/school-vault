...
static HWSize_t WriteWordDocSetFn(FILE *f,
  ...
  uint16_t wordlen_ho = strlen(wds->word);
  ...
  res = fwrite(wds->word, sizeof(*wds->word), wordlen_ho, f);
  if (res != wordlen_ho) {
    return 0;
  }
...
static HWSize_t WriteHeader(FILE *f,
  ...
  // Get bytes for checksum.
  uint8_t next;
  for (HWSize_t i = 0; i < cslen; ++i) {
    res = fread(&next, sizeof(next), 1, f);
    if (res == 0) {
      return 0;
    }
...
static HWSize_t WriteHashTable(FILE *f,
  ...
  for (i = 0; i < ht->num_buckets; i++) {
    ...
    res = WriteBucketRecord(f, bucket, next_bucket_rec_offset,
                            next_bucket_offset);
    if (res == 0) {
      return 0;
    }

    res = NumElementsInLinkedList(bucket);
    if (res > 0) {
      res = WriteBucket(f, bucket, next_bucket_offset, fn);
      if (res == 0) {
        return 0;
      }
