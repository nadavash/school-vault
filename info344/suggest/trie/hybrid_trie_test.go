...
func TestHybridTrieConstructor(t *testing.T) {
	tr := trie.NewHybridTrie()
	if tr == nil {
...
func TestHybridTrieFromStream(t *testing.T) {
	tr, err := loadHybridTrieFromFile()
	if err != nil {
		t.Fatal(err)
	}
...
func BenchmarkHybridPrefixSearch(b *testing.B) {
...
func loadHybridTrieFromFile() (*trie.HybridTrie, error) {
	file, err := os.Open(resourceFile)
	if err != nil {
		return nil, err
	}
	tr := trie.NewHybridTrieFromStream(file)
	file.Close()
